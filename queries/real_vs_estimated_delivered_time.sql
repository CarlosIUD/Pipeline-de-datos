...
SELECT
	...
	ROUND(AVG(JULIANDAY(order_delivered_customer_date) - JULIANDAY(order_purchase_timestamp)),
	2) AS real_time,
	ROUND(AVG(JULIANDAY(order_estimated_delivery_date) - JULIANDAY(order_purchase_timestamp)),
	2) AS estimated_time
FROM
	olist_orders
WHERE
...
SELECT
	m.month_no,
	m.month,
	COALESCE(SUM(CASE WHEN d.year = '2016' THEN d.real_time END),
	NULL) as Year2016_real_time,
	COALESCE(SUM(CASE WHEN d.year = '2017' THEN d.real_time END),
	NULL) as Year2017_real_time,
	COALESCE(SUM(CASE WHEN d.year = '2018' THEN d.real_time END),
	NULL) as Year2018_real_time,
	COALESCE(SUM(CASE WHEN d.year = '2016' THEN d.estimated_time END),
	NULL) as Year2016_estimated_time,
	COALESCE(SUM(CASE WHEN d.year = '2017' THEN d.estimated_time END),
	NULL) as Year2017_estimated_time,
	COALESCE(SUM(CASE WHEN d.year = '2018' THEN d.estimated_time END),
	NULL) as Year2018_estimated_time
FROM
	MonthNames m
LEFT JOIN DeliveryTimes d ON
	m.month_no = d.month_no
GROUP BY
	m.month_no
ORDER BY
	m.month_no;